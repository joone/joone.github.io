...
When I first encountered ChatGPT, I thought it would be a good idea to use it to write Git commit messages.
...
In interactive mode, you can start a conversation with ChatGPT by typing your query and pressing Enter.
...
Note that the author, date, and commit ID lines are stripped from the commit message before it is sent to the OpenAI server.

### Find Chat History

To access chat history, look for the `.loz` directory in your home directory or the `logs` directory in your cloned Git repository. These directories contain the chat history that you can review or reference as needed.

However, the OpenAI API is not free to use. The results of the OpenAI API also differ from those of ChatGPT, being more concise. If you have already set up a payment method for the OpenAI API,
