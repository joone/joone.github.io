...
- [1144255](https://chromium-review.googlesource.com/c/chromium/src/+/1144255) window-service: removes `mus_demo`, `test_wm`, and some unnecessary deps
- [Issue 865689](https://bugs.chromium.org/p/chromium/issues/detail?id=865689) Remove window-service

I used to run `mus_demo` to test ozone-gbm, but now it's gone. :-( So I need to figure out what the difference between WS1 and WS2 is.

What is the Window Service of ChromeOS? The Window Service is a kind of window compositor like Weston. It is a separate process from the browser process in CrOS. Some CrOS features are part of the browser process today, but those features will be moved into a separate process using the WindowService.
